class Cart {
  ...
  deleteProduct(id) {
    this.cart[id] > 1 ? this.cart[id] -= 1 : delete this.cart[id];
    this.saveCart();
    this.updateBadge();
  }
  createCartProductDomString(product) {
    return `<div class="row" data-id="${product.id}"> 
                      ...
                      <div class="col-1"><button data-id=${product.id
  ...
  updateBadge() {
    if (Object.keys(this.cart).length) {
      document.querySelector(".cart-badge").innerHTML = Object.values(this.cart).reduce((a, b) => a + b);
      document.querySelector(".navigation__cart").style.display = "block";
      return

    }
    document.querySelector(".navigation__cart").style.display = "none";
  }
  async order(ev) {
